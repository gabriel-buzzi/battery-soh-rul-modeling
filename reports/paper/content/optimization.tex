...
Instead of optimizing a single performance metric, a \textit{multi-objective} approach was adopted to balance predictive accuracy and overfitting. The two objectives minimized simultaneously are:

\begin{itemize}
  \item The \gls{rmse} on the validation folds, as defined by \cref{eq:rmse}
  \item The relative generalization gap between training and validation errors:
    \[
    \text{RelativeGap} = \frac{|\text{RMSE}_{\text{train}} - \text{RMSE}_{\text{val}}|}{\text{RMSE}_{\text{val}}}
    \]
\end{itemize}

This formulation favors models that achieve low error on the validation data while maintaining consistency with training performance, discouraging overfitting. Trials were evaluated using 5-fold Group K-Fold cross-validation. The 99 training cells were partitioned into 5 non-overlapping groups (based on cell ID), ensuring that all cycles from a given cell remained within the same fold, thus preventing data leakage. The final score for each objective is computed as the average of metrics across the 5 validation folds. \cref{fig:kfold} below illustrates the 5-fold cross-validation process adopted.

To improve optimization efficiency and prevent unnecessary trials, an early stopping strategy was employed, so if the Pareto front remained unchanged for a fixed number of consecutive trials (patience = 10), the optimization process was halted. This ensures that exploration ceases when no meaningful improvement is observed across both objectives.

By combining a multi-objective strategy with Pareto-based early stopping, the optimization process effectively identified well-generalized models while minimizing computational cost.

\insertfigure{optimization/kfold}

% Summarizing the methodology
This methodology leverages full-life data from training cells to develop models that generalize to unseen cells, addressing the challenges of \gls{soh} and \gls{rul} estimation. Through standardized features, multi-object optimization and hyperparameter optimization via TPE and cross-validation, the approach ensures robust predictions avoiding artificially inflated metrics due to overfitting.
