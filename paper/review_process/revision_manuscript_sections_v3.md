...
`COMMENT: I think this part is still a little confusing, it's not clear that the full-cycle base is used only for the full cycle analyses and that the charge-only baseline is used for the charge-only analyses. Please confirm that this is acctually what is happening in the code. ANSWER: I confirmed this in the code. The full-cycle baseline is used only for the full-cycle analyses, and the charge-only analyses use their own independently optimized charge-only baseline. I clarified that explicitly below.`
...
`COMMENT: What exactly are these chage statiscts mentioned and which of them were in fact used to group the cells and how ANSWER: Implemented below by stating the exact statistics used.`
`COMMENT: Since the resulting groups are all no_rest, does it worth mentioning this criteria in the text/tables? ANSWER: I kept a brief mention for reproducibility, because it is part of the implemented grouping logic, but I reduced its analytical emphasis since it does not separate the evaluated families in this artifact batch.`
...
`COMMENT: Explicitly mention that the C-rate does not seem to be correlated with the cycle life in most cells ANSWER: Implemented below by clarifying that the family-level cycle-life pattern is not monotonic with charge C-rate, so C-rate alone does not explain the observed lifetime variability.`
...
`COMMENT: I did not find the leave-one-feature-out results for the selected k value in the text, are this results available in the artifacts? ANSWER: Yes. They are available in the saved `loo_metrics.csv` artifacts, and I added a short results paragraph below summarizing the main leave-one-feature-out findings for the selected subsets.`
## Baseline Full-Cycle Performance

Table 1 summarizes the four baseline configurations used as anchors for the later analyses. The held-out test performance confirms that the feature-based `ExtraTrees` approach remains viable for cross-cell prognosis, although the two targets are not equally difficult and the full-cycle view remains stronger than the charge-only view. SoH estimation remains comparatively accurate in both settings, with test RMSE of 1.07 percentage points for the full-cycle baseline and 1.15 for the charge-only baseline, both with strong test-set $R^2$. By contrast, cycle-based RUL prediction is substantially harder, with test RMSE of 145.66 cycles for the full-cycle baseline and 146.47 cycles for the charge-only baseline, together with a notably larger train-validation gap already visible in cross-validation. This asymmetry indicates that the single-cycle statistics capture present health more directly than long-horizon lifetime, while the charge-only view preserves much of the SoH signal but leaves less margin for robust RUL estimation.
...
Source artifacts: `baseline_evaluation/*/table_main_metrics.csv`
...
`COMMENT: At the methodology we talked about the intrinsic model importance, but I can't see any mentions to this in the results, is this data available in the artifacts? If not, would it be more valuable to obtain it or to simply remove the mention to it? ANSWER: The intrinsic-importance files are available in the artifacts. I added a short comparison below showing that they broadly support the permutation ranking, so the methodology mention remains justified without expanding the paper into a second parallel ranking analysis.`
...
`COMMENT: Add the complete rank to this table, not only the top-6, if possible ANSWER: Implemented below.`
...
The new held-out test runs partially support this compactness claim, but also add an important qualification. For SoH, the six-feature full-cycle subset remains reasonably competitive on unseen cells, with test RMSE 1.1662 and test $R^2 = 0.9292$, but it does not match the 16-feature baseline. For RUL, the same compactness move is more costly on the held-out cells: test RMSE rises from 145.66 to 155.46 cycles and test $R^2$ falls from 0.8660 to 0.8473. The compact full-cycle models should therefore be interpreted as deployable approximations that preserve much of the baseline behavior, especially for SoH, rather than as uniformly stronger alternatives to the full 16-feature models.
...
The no-temperature ablation further strengthens the conclusion that temperature contributes weakly and inconsistently in this dataset. Removing all temperature features improved validation RMSE for both full-cycle SoH and full-cycle RUL. On the held-out test set, the no-temperature SoH variant also improved over the full baseline, reducing test RMSE from 1.0660 to 1.0251 and increasing test $R^2$ from 0.9409 to 0.9453. For RUL, however, the held-out behavior is more mixed: cross-validation improves substantially, but test RMSE worsens slightly from 145.66 to 153.14 cycles and test $R^2$ falls from 0.8660 to 0.8519. The original dataset documentation reports problems with thermocouple attachment and stability, which likely contributes noise to temperature-derived statistics. Taken together, the results still indicate that temperature is not a reliable pillar of the present feature representation, but they also show that validation gains do not translate uniformly into held-out RUL gains once the full-cycle model is tested on unseen cells.

**Table 4. Full-cycle no-temperature ablation. Columns compare the full 16-feature baseline and the corresponding no-temperature variant in terms of number of features, validation RMSE, mean train-validation relative gap, optimization objective score, and held-out test performance.**  
Source artifacts: `full_cycle_feature_analysis/*/no_temp_metrics.json`, `baseline_evaluation/*/table_main_metrics.csv`

| Target | Configuration | n_features | Val RMSE | Relative gap | Objective score | Test RMSE | Test MAE | Test R2 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| SOH | Full-cycle baseline | 16 | 1.1433 | 0.3333 | 1.4766 | 1.0660 | 0.8037 | 0.9409 |
| SOH | No-temperature | 11 | 1.0869 | 0.2657 | 1.3526 | 1.0251 | 0.7718 | 0.9453 |
| RUL | Full-cycle baseline | 16 | 112.0319 | 0.7339 | 112.7658 | 145.6603 | 89.5122 | 0.8660 |
| RUL | No-temperature | 11 | 101.9910 | 0.5853 | 102.5763 | 153.1365 | 89.4077 | 0.8519 |
...
`COMMENT: Add the full rank to these tables instead of just the top-6 ANSWER: Implemented below.`
...
The held-out compact-subset runs sharpen this comparison. For charge-only SoH, the six-feature compact model slightly outperforms the 16-feature charge-only baseline on unseen cells, reducing test RMSE from 1.1532 to 1.0780 and raising test $R^2$ from 0.9308 to 0.9395. This suggests that, in the charge-only SoH setting, pruning weak or noisy variables is beneficial not only in validation but also in held-out evaluation. For charge-only RUL, the four-feature compact model remains workable but loses accuracy relative to the 16-feature charge-only baseline, with test RMSE increasing from 146.47 to 150.60 cycles and test $R^2$ dropping from 0.8645 to 0.8567. As in the full-cycle case, compact charge-only RUL should therefore be framed as a simpler approximation rather than as a superior model.
...
**Table 7. Charge-only no-temperature ablation. Columns compare the full 16-feature charge-only baseline and the corresponding no-temperature variant in terms of number of features, validation RMSE, mean train-validation relative gap, optimization objective score, and held-out test performance.**  
Source artifacts: `charge_only_feature_analysis/*/no_temp_metrics.json`, `baseline_evaluation/*/table_main_metrics.csv`

| Target | Configuration | No. of features | Validation RMSE | Relative gap | Objective score | Test RMSE | Test MAE | Test R2 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| SOH | All charge-only features | 16 | 1.2178 | 0.2459 | 1.4638 | 1.1532 | 0.8352 | 0.9308 |
| SOH | Charge-only without temperature features | 11 | 1.2277 | 0.2254 | 1.4530 | 1.0414 | 0.7546 | 0.9436 |
| RUL | All charge-only features | 16 | 136.0298 | 0.7673 | 136.7971 | 146.4735 | 95.9620 | 0.8645 |
| RUL | Charge-only without temperature features | 11 | 130.5267 | 0.6692 | 131.1959 | 128.7977 | 84.8248 | 0.8952 |
...
`COMMENT: Break this table into two, one for each target. And do not repeat information from table M2 here ANSWER: Implemented below as Tables 10a and 10b, keeping only the family label and the predictive metrics.`
...
`COMMENT: Make at some point a parallel with the battery industry, explaining what the observed performance would mean in an real-world scenario ANSWER: Implemented below by adding an industry-oriented interpretation paragraph in the discussion.`
...
The charge-only results further support this interpretation. Restricting the analysis to charging segments degrades predictive accuracy, especially for RUL, but does not destroy the prognostic signal. The held-out results are particularly informative here: charge-only SoH remains close to the full-cycle baseline, and the compact six-feature and no-temperature eleven-feature charge-only SoH variants even improve over the 16-feature charge-only baseline on unseen cells. This suggests that partial-cycle prognosis is feasible when operational constraints make discharge data unavailable, provided that the feature set is pruned carefully. At the same time, the quantitative gap relative to full-cycle performance remains substantial for RUL, so charge-only variants should still be framed as practical approximations rather than as equivalent substitutes.
