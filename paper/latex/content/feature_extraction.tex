...
\begin{enumerate}
    ...
    \item \textbf{Smoothing:} A first-order Savitzky-Golay filter with a 10-cycle window was used to dampen cycle-to-cycle oscillations, which do not reflect the gradual evolution of \ac{soh} and \ac{rul}.
\end{enumerate}

The processed features, shown in \cref{fig:features_with_filter}, retain degradation trends while reducing noise and anomalies.

\insertfigure{features_extraction_features}
\insertfigure{features_extraction_features_with_filter}
...
Pearson correlation coefficients between features and targets (\ac{soh} and \ac{rul}), shown in \cref{fig:correlation_heatmap}, highlight strong relationships, especially among voltage features, making them key candidates for modeling.

\insertfigure{features_extraction_correlation_heatmap}
